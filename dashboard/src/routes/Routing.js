...
const Routing = () => {
  ...
  async function test() {
    ...
    const DuBalance = await web3.eth.getBalance(regAddress);
    const contract = new web3.eth.Contract(cont_abi, cont_address);

    let min_stakeAmount = await contract.methods.minimum_investment().call();
    let min_Withlimit = await contract.methods
      .minimum_withdraw_reward_limit()
      .call();
    let max_Withlimit = await contract.methods
      .maximum_withdraw_reward_limit()
      .call();
    ...
    let TotalInvestment = await contract.methods
      .getTotalInvestment()
      .call({ from: regAddress });
    let totalEarning;
    let referralLevel_earning;
    try {
      totalEarning = await contract.methods
        .get_totalEarning()
        .call({ from: regAddress });
    } catch {}

    let history = await contract.methods
      .get_history(regAddress)
      .call({ from: regAddress });
    let referralLevel_count = await contract.methods
      .Level_count(regAddress)
      .call();
    try {
      referralLevel_earning = await contract.methods
        .Level_earning(regAddress)
        .call();
    } catch {}
        ...
        <Route path="dashboard">
          ...
          <Route
            ...
            element={
              <ProtectedRoute>
                <Reward
                  referralLevel_count={referralLevel_count}
                  referralLevel_Earning={referralLevel_Earning}
                />
              </ProtectedRoute>
            }
          />
