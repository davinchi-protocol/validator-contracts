  ...
  async function handleRegister(event) {
    ...
    try {
      response = await Axios.get(
        "https://api.davinci.bz/getdatabymail?" +
        new URLSearchParams({
          Email: email,
        }),
      );
    ...
    try {
      userData = await Axios.get(
        ...
        new URLSearchParams({ userAddress: address.toLowerCase() }),
      );

    } catch (e) {
    }
    if (userData.data.status === 409) {
      toast("Your Wallet Address is already Registered");
      allow_reg = true;
      return;
    }
    if (password.length < 8) {
      toast("Password length cannot be less than 8 characters");
      allow_reg = true;
    ...
    if (country == "Select Country") {
      toast("Select the country");
      allow_reg = true;

      return;
    }
    if (!isValidAddress(address)) {
      toast("Enter the valid Ethereum Address");
      allow_reg = true;
    ...
    if (
      ...
      let userData1;
      try {
        userData1 = await Axios.get(
          "https://api.davinci.bz/getdatabyaddress?" +
          new URLSearchParams({ userAddress: ref.toLowerCase() }),
        );
      } catch (e) {
      }
      if (userData1.status === 400) {
        toast("Referral Address' is not registered");
        allow_reg = true;
        return;
      }
    } else {
    ...
    try {
      await Axios.post("https://api.davinci.bz/register", {
        ...
        alert("Registration is successful, Please login to continue");
        navigate("/");
      });
    } catch (e) {
      allow_reg = true;
