...
const Verification = (props) => {
  ...
  async function upload() {
    if (preview == "") {
      toast("kindly upload the image");
      return;
    }
    const res0 = await axios.get(
      "https://api.davinci.bz/getdatabyaddress?" +
        new URLSearchParams({ userAddress: props.regAddress }),
    );

    if (res0.data[0] != undefined) {
      if (res0.data[0].verified == "undefined") {
        const data = {
          userAddress: res0.data[0].userAddress.toLowerCase(),
          FName: res0.data[0].FName,
          LName: res0.data[0].LName,
          Email: res0.data[0].Email,
          password: res0.data[0].password,
          Country: res0.data[0].Country,
          Phone: res0.data[0].Phone,
          Ref_address: res0.data[0].Ref_address,
          verified: "verified",
          Image: preview,
        };

        const res = await axios.patch(
          "https://api.davinci.bz/user/" + res0.data[0]._id,
          data,
        );
  ...
  async function check() {
    const res0 = await axios.get(
      "https://api.davinci.bz/getdatabyaddress?" +
        new URLSearchParams({ userAddress: props.regAddress.toLowerCase() }),
    );

    if (res0.data[0].Image == "null" && res0.data[0].verified == "undefined") {
      set_status(0);
    } else if (
      res0.data[0].Image == "null" &&
      res0.data[0].verified == "underApproval"
    ) {
      // set_status(1)
      set_status(0);
    } else if (
      ...
      res0.data[0].Image != "null" &&
      res0.data[0].verified == "verified"
    ) {
      set_status(2);
    } else if (
      res0.data[0].Image != "null" &&
      res0.data[0].verified == "decline"
    ) {
      set_status(3);
    }
  }
            ...
            {status == 0 ? (
              <div className="flex flex-col justify-center mt-12 verify max">
                ...
                <div className="flex btns justify-center items-center gap-6 mt-7 max-md:gap-4">
                  ...
                  <button className="sub" onClick={upload}>
                    Submit
                  </button>
                </div>
              </div>
              ...
              <div className="Verification-box">
                <div className="flex flex-col justify-center mt-20 verify max">
                  <h2 className="veri-heading">KYC Verification</h2>
                  <p className="veri-desc ">
                    Your Request is under processing, you will get the response
                    in next 2 business days
                  </p>
                </div>
              </div>
            ) : status == 2 ? (
              <div className="Verification-box">
                <div className="flex flex-col justify-center mt-20 verify max">
                  <h2 className="veri-heading" style={{ color: "#EB8C55" }}>
                    Congratulations, You are a part of (DCOIN) Community !
                  </h2>
                  <p className="veri-desc " style={{ color: "green" }}>
                    Your Request has been approved, you can now stake $DCOIN and
                    can earn Reward.
                  </p>
                </div>
              </div>
              ...
              <div className="Verification-box">
                <div className="flex flex-col justify-center mt-20 verify max">
                  <h2 className="veri-heading" style={{ color: "red" }}>
                    We are Sorry to inform that Your request has been declined!
                  </h2>
                </div>
              </div>
            ) : null}
